...
class ResultLogger:
    ...
    def __init__(self, tag, logdir='.', verbose=False):
        ...
        # training metric
        self.training_loss = []
        self.training_encoder_loss = []
        self.training_decoder_loss = []
        self.training_discriminator_loss = []
    ...
    def add_training_metrics(self, avg_encoder_loss, avg_decoder_loss, avg_discriminator_loss, accuracy, time=0.):
        self.training_encoder_loss.append(avg_encoder_loss)
        self.training_decoder_loss.append(avg_decoder_loss)
        self.training_discriminator_loss.append(avg_discriminator_loss)
        self.training_accuracies.append(accuracy)
        self.training_time.append(time)

    def save_latent_code(self, epoch: int, z: np.ndarray, y: np.ndarray):
    ...
    def save_prediction(self, epoch: int, labels: np.ndarray, predicts: np.ndarray, probs: np.ndarray, time=0.):
    ...
    def save_metrics(self):
        ...
        titles3 = ['encloss', 'decloss', 'discloss', 'accuracy', 'time']
        for i, title in enumerate(titles3):
            sheet3.write(0, i, title)
        for i in range(len(self.training_encoder_loss)):
            row = i + 1
            sheet3.write(row, 0, self.training_encoder_loss[i])
            sheet3.write(row, 1, self.training_decoder_loss[i])
            sheet3.write(row, 2, self.training_discriminator_loss[i])
            sheet3.write(row, 3, self.training_accuracies[i])
            sheet3.write(row, 4, self.training_time[i])
